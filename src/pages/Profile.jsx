...
const Profile = () => {
  ...
  async function onSubmit() {
    try {
      ...
      if (auth.currentUser.displayName !== name) {
        ...
        //update display name in fire-store db.
  ...
  //When Profile page is loaded we want to fetch all the listing from database
  //We have to use useEffect here.
  useEffect(() => {
  ...
  async function onDelete(listingID) {
    if (window.confirm("Are you sure you want to delete")) {
      ...
      const updateedListings = listings.filter(
        (listing) => listing.id !== listingID
      );
      setListings(updateedListings);
    ...
    <>
      <section className="max-w-6xl mx-auto flex justify-center items-center flex-col">
        ...
        <div className="w-full md:w-[50%] mt-6 px-3">
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white uppercase py-3 px-7 text-sm
             font-medium
